...
export async function buildAlerts(
  ...
  try {
    ...
    for (const target of targets) {
      const currentPrice = prices[target.name]?.usd;
...
export async function sendAlerts(alerts: Alert[]): Promise<boolean> {
  for (const alert of alerts) {
    try {
      const channel = (await discordClient.channels.fetch(alert.channelId)) as
        | DMChannel
        | PartialDMChannel
        | null;
      if (channel) {
        ...
        if (alert.basis) {
          ...
          message += `\n💰 This is a profit of $${profit.toFixed(2)} compared to ${alert.percentage}% of your initial investment.`;
        }
